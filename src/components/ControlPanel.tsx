...
export default function ControlPanel({ settings, onChange, modelInfo }: ControlPanelProps) {
  return (
    <div className="flex flex-col">
      ...
      <Toggle label="Ground Grid" checked={settings.showGrid} onChange={v => onChange({ showGrid: v })} />

      {settings.autoRotate && (
        <Slider label="Rotation Speed" value={settings.autoRotateSpeed} min={0.1} max={5} step={0.1} onChange={v => onChange({ autoRotateSpeed: v })} />
      )}

      <SectionTitle>Appearance</SectionTitle>
      <Slider label="Brightness" value={settings.exposure} min={0.3} max={3} step={0.1} onChange={v => onChange({ exposure: v })} />
      <Slider label="Light Power" value={settings.lightIntensity} min={0} max={5} step={0.1} onChange={v => onChange({ lightIntensity: v })} />
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {([
              ...
              <div key={label} className="rounded-xl bg-[var(--chip-bg)] px-3 py-2">
                ...
                <p className="text-sm font-semibold tabular-nums text-[var(--sea-ink)]">{val}</p>
              </div>
            ))}
          </div>
        </>
